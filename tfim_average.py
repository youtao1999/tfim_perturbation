...
for i, h_x in enumerate(h_x_range):
    outF.write("{index} {h_x_val} {GS_energy_val} {first_excited_energy_val} {second_derivative_energy_val} {susceptibility_val} {structure_factor_val} \n".format(index = i, h_x_val = h_x, GS_energy_val = exc_eigenvalues_ave[i], first_excited_energy_val = first_excited__exc_energies_ave[i], second_derivative_energy_val = second_derivative_exc_eigenvalues_ave[i], susceptibility_val = chi_arr_ave[i], structure_factor_val = S_SG_arr_ave[i]))
